...
class Trajectory():
    ...
    @staticmethod
    def _compute_body_axes(accel_vec, vel, yaw_offset, gravity_vector):
        thrust_vec = accel_vec - gravity_vector.reshape(1, 3)
        b3 = Trajectory._safe_normalize_rows(thrust_vec)

        vel_dir = Trajectory._safe_normalize_rows(vel)
        vel_proj = vel_dir - (np.sum(vel_dir * b3, axis=1, keepdims=True) * b3)
        vproj_norm = np.linalg.norm(vel_proj, axis=1, keepdims=True)

        fallback = np.tile(np.array([1.0, 0.0, 0.0]), (b3.shape[0], 1))
        fallback_proj = fallback - (np.sum(fallback * b3, axis=1, keepdims=True) * b3)
        use_fallback = vproj_norm.squeeze() < 1e-8
        if np.any(use_fallback):
            vel_proj[use_fallback] = fallback_proj[use_fallback]

        x_ref = Trajectory._safe_normalize_rows(vel_proj)

        cos_yaw = np.cos(yaw_offset)
        sin_yaw = np.sin(yaw_offset)
        dot_b3_x = np.sum(b3 * x_ref, axis=1, keepdims=True)
        cross_b3_x = np.cross(b3, x_ref)
        x_ref_rot = (
            x_ref * cos_yaw[:, None]
            + cross_b3_x * sin_yaw[:, None]
            + b3 * dot_b3_x * (1.0 - cos_yaw)[:, None]
        )
    ...
    def sample_full_state(
        ...
        b1, b2, b3 = self._compute_body_axes(acc, vel, yaw_offset, gravity_vector)
...
if __name__ == "__main__":
    ...
    print(
        "boundary continuity violation (should be near zero):",
        np.linalg.norm(np.array(segment._end_node.con_x) - np.array(segment2._start_node.con_x)),
        np.linalg.norm(np.array(segment._end_node.con_y) - np.array(segment2._start_node.con_y)),
        np.linalg.norm(np.array(segment._end_node.con_z) - np.array(segment2._start_node.con_z)),
        np.linalg.norm(np.array(segment._end_node.con_psi) - np.array(segment2._start_node.con_psi)),
        )
