...
class WaypointTemplate():
    ...
    def sample_with_vels(self):
        xyzs = self._randomized_xyzs()
        rpys = self._randomized_rpys()
        vels = np.asarray(self.waypoints_vels, dtype=float).copy()
        if vels.shape[0] != xyzs.shape[0]:
            raise ValueError("waypoints_vels must have the same length as waypoints_xyzs")
        return xyzs, rpys, vels, self.spawns, self.max_dist

    def sample_with_vels_and_durations(self):
        xyzs = self._randomized_xyzs()
        rpys = self._randomized_rpys()
        vels = np.asarray(self.waypoints_vels, dtype=float).copy()
        ...
        return xyzs, rpys, vels, durations, self.spawns, self.max_dist
    ...
    def _randomized_xyzs(self):
        if self.xyzs_normal_distr is None:
            return np.asarray(self.waypoints_xyzs, dtype=float)
    ...
    def _randomized_rpys(self):
        if self.rpy_choices is None:
            return np.asarray(self.waypoints_rpys, dtype=float)
        waypoints_rpys = np.asarray(self.waypoints_rpys, dtype=float)
