...
from isaaclab_tasks.direct.acro_drone.trajectory.acro_templates import HeartTemplate, PowerloopTemplate, SplitSLeftTemplate, BarrelRollLeftTemplate
...
cfg = MpcCfg(
	...
	w_body_rate=0.0,
	w_output=0.0,
	w_output_derivative=0.0,
	max_normalized_thrust=60.0,
	max_roll_pitch_rate=25.0,
	max_yaw_rate=10.0,
)
num_envs = 2
...
templates: List[WaypointTemplate] = [BarrelRollLeftTemplate()]
...
x0 = np.zeros((num_envs, 13), dtype=float)
...
for i in range(num_steps):
	...
	sim_state = np.asarray(simulator.make_step(u0[0])).reshape(13,)
	x0[0] = sim_state
